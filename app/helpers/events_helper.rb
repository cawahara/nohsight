module EventsHelper
   ...
   def get_nearby_locations(location)
      begin
         ...
         place_api_uri = URI.parse("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=#{geo_info['latitude']},#{geo_info['longitude']}&radius=2000&keyword=temple&language=ja&key=#{ENV['GOOGLE_API_KEY']}")
         ...
         3.times do |n|
            ...
            if place_title != nil
               ...
               place_thumbnail = Net::HTTP.get(URI.parse("https://maps.googleapis.com/maps/api/place/photo?&sensor=false&maxwidth=80&maxheight=80&photoreference=#{place_json['results'][n]['photos'][0]['photo_reference']}&key=#{ENV['GOOGLE_API_KEY']}"))
                  # FIXME 上記のリクエストだとページそのものにリダイレクトし、画像そのものを引き出せない
         ...
         return rec_places
      rescue NoMethodError    # 周辺に関連スポットがないときに発生
         return []
      end
   end
end
