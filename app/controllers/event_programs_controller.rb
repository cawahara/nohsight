...
class EventProgramsController < ApplicationController
   ...
   def update
      ...
      event_program_input.each do |event_params|
         next if event_program_valid?(event_params) != false
      ...
      event_program_input.each do |event_params|
         ...
         when 'update'
            ...
            event_program.update_attributes(program_id: event_params['title'],
         ...
         when 'create'
            event_program = @event.event_programs.build(program_id: event_params['title'],
                                                        style:      event_params['style'],
                                                        genre:      event_params['genre'])
            event_program.save
         end
