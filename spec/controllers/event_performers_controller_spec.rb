   ...
   describe 'PATCH #update' do
      ...
      context 'with valid param in create action' do
         ...
         before(:each) do
            ...
            @ev_per_params = {'0': { type: 'create', id: '', event_program_id: event_program.id, full_name: diff_performer[:full_name] } }
         end

         it 'creates a new event program' do
            expect{ patch :update, id: event, event_program: @ev_pro_params, "event_performer-0": @ev_per_params }.to change(EventPerformer, :count).by(1)
         end
         ...
         it 'creates a new performer when a new name is init' do
            new_ev_per_params = {'0': { type: 'create', id: '', event_program_id: event_program.id, full_name: 'New Performer' } }
            expect{ patch :update, id: event, event_program: @ev_pro_params, "event_performer-0": new_ev_per_params }.to change(Performer, :count).by(1)
         end
      ...
      context 'with valid param in update action' do
         ...
         before(:each) do
            login_as(user)
            @ev_pro_params = {'0': { type: 'update', id: event_program.id } }
            @ev_per_params = {'0': { type: 'update', id: event_performer.id, event_program_id: event_program.id, full_name: diff_performer[:full_name] } }
            patch :update, id: event, event_program: @ev_pro_params, "event_performer-0": @ev_per_params
         end
         ...
         it 'creates a new performer when a new name is init' do
            new_ev_per_params = {'0': { type: 'update', id: event_performer.id, event_program_id: event_program.id, full_name: 'New Performer' } }
            expect{ patch :update, id: event, event_program: @ev_pro_params, "event_performer-0": new_ev_per_params }.to change(Performer, :count).by(1)
         end
      ...
      context 'with valid param in destroy action' do
         ...
         before(:each) do
            login_as(user)
            @ev_pro_params = {'0': { type: 'update', id: event_program.id } }
            @ev_per_params = {'0': { type: 'destroy', id: event_performer.id, event_program_id: event_program.id } }
         end
      ...
      context 'without login' do
         ...
         it "doesn't create a new event performer" do
            ev_per_params = {'0': { type: 'create', id: '', event_program_id: event_program.id, full_name: diff_performer[:full_name] } }
            expect{ patch :update, id: event, event_program: @ev_pro_params, "event_performer-0": ev_per_params }.to change(EventPerformer, :count).by(0)
         end
      ...
      context 'with invalid or empty required param' do
         ...
         it "doesn't create a new event performer" do
            ev_per_params = {'0': { type: 'create', id: event_performer.id, event_program_id: event_program.id, full_name: '' } }
            expect{ patch :update, id: event, event_program: @ev_pro_params, "event_performer-0": ev_per_params }.to change(EventPerformer, :count).by(0)
         end

         it "doesn't update a event performer" do
            ev_per_params = {'0': { type: 'update', id: event_performer.id, event_program_id: event_program.id, full_name: '' } }
            patch :update, id: event, event_program: @ev_pro_params, "event_performer-0": ev_per_params
