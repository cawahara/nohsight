...
RSpec.describe EventProgramsController, type: :controller do
   ...
   describe 'PATCH #update' do
      ...
      context 'with valid param in create action' do
         ...
         before(:each) do
            login_as(user)
            @ev_pro_params = {'0': { type: 'create', id: '', title: diff_program[:title], genre: 'Genre', style: 'Style' }}
         end

         it 'creates a new event program' do
            expect{ patch :update, id: event, event_program: @ev_pro_params }.to change(EventProgram, :count).by(1)
         end
         ...
         it 'creates a new program when a new title is init' do
            new_ev_pro_params = {'0': { type: 'create', id: '', title: 'New Program', genre: 'Genre', style: 'Style' }}
            expect{ patch :update, id: event, event_program: new_ev_pro_params }.to change(Program, :count).by(1)
         end
      ...
      context 'with valid param in update action' do
         ...
         before(:each) do
            login_as(user)
            @ev_pro_params = {'0': { type: 'update', id: event_program.id, title: diff_program[:title], genre: 'Genre', style: 'Style' }}
            patch :update, id: event, event_program: @ev_pro_params
         end
         ...
         it 'creates a new program when a new title is init' do
            new_ev_pro_params = {'0': { type: 'update', id: event_program.id, title: 'New Program', genre: 'Genre', style: 'Style' }}
            expect{ patch :update, id: event, event_program: new_ev_pro_params }.to change(Program, :count).by(1)
         end
      ...
      context 'without login' do
         ...
         it "doesn't create a new event program" do
            ev_pro_params = {'0': { type: 'create', id: event_program.id, title: diff_program[:title], genre: 'Genre', style: 'Style' }}
            expect{ patch :update, id: event, event_program: ev_pro_params }.to change(EventProgram, :count).by(0)
         end

         it "doesn't update the event program" do
            ev_pro_params = {'0': { type: 'update', id: '', title: diff_program[:title], genre: 'Genre', style: 'Style' }}
            patch :update, id: event, event_program: ev_pro_params
            event_program.reload
      ...
      context 'with invalid or empty required param' do
         ...
         it "doesn't create a new event program" do
            ev_pro_params = {'0': { type: 'create', id: '', title: diff_program[:title], genre: '', style: '' }}
            expect{ patch :update, id: event, event_program: ev_pro_params }.to change(EventProgram, :count).by(0)
         end

         it "doesn't update a event program" do
            ev_pro_params = {'0': { type: 'update', id: event_program.id, title: diff_program[:title], genre: '', style: '' }}
            patch :update, id: event, event_program: ev_pro_params
            event_program.reload
            expect(event_program.program_id).not_to eq(diff_program.id)
         end
